...
; (function ($w) {
    ...
    jm.node = function (sId, iIndex, sTopic,link, oData, bIsRoot, oParent, eDirection, bExpanded) {
    ...
    jm.mind.prototype = {
        ...
        insert_node_before: function (node_before, nodeid, topic,link, data) {
            if (!jm.util.is_node(node_before)) {
                ...
                if (!the_node_before) {
                    ...
                    return this.insert_node_before(the_node_before, nodeid, topic,link, data);
                }
            }
        ...
        insert_node_after: function (node_after, nodeid, topic,link,data) {
            if (!jm.util.is_node(node_after)) {
                ...
                if (!the_node_after) {
                    ...
                    return this.insert_node_after(the_node_after, nodeid, topic,link,data);
                }
            }
            ...
            _extract_root: function (mind, node_array) {
                ...
                while (i--) {
                    if ('isroot' in node_array[i] && node_array[i].isroot) {
                        ...
                        mind.set_root(root_json.id, root_json.topic,link,data);
    ...
    jm.prototype = {
        ...
        insert_node_before: function (node_before, nodeid, topic,link,data) {
            if (this.get_editable()) {
                ...
                var node = this.mind.insert_node_before(node_before, nodeid, topic,link,data);
                if (!!node) {
                    this.view.add_node(node);
                    this.layout.layout();
                    this.view.show(false);
                    this.invoke_event_handle(jm.event_type.edit, { evt: 'insert_node_before', data: [beforeid, nodeid, topic,link,data], node: nodeid });
                }
                return node;
            } else {
                logger.error('fail, this mind map is not editable');
        ...
        insert_node_after: function (node_after, nodeid, topic,link,data) {
            if (this.get_editable()) {
                ...
                var node = this.mind.insert_node_after(node_after, nodeid, topic,link,data);
                if (!!node) {
                    this.view.add_node(node);
                    this.layout.layout();
                    this.view.show(false);
                    this.invoke_event_handle(jm.event_type.edit, { evt: 'insert_node_after', data: [afterid, nodeid, topic,link,data], node: nodeid });
                }
                return node;
            } else {
                logger.error('fail, this mind map is not editable');
    ...
    jm.view_provider.prototype = {
        ...
        create_node_element: function (node, parent_node) {
			...
			if(link){
				...
				var new_html = "<a style='color:"+link.rgb+";' onclick=controller.C_tree_link_click('" +link.a+ "','"+link.subject +"','"+ link.moduel + "','"+link.chapter+ "','"+link.topic+ "')>"+link.pct +' ' + org_html +"</a>";
        ...
        zoomInMax: function () {
            return this.setZoom(2);
        }, zoomIn: function () {
            return this.setZoom(this.actualZoom + this.zoomStep);
        },

        zoomOutMax: function () {
            return this.setZoom(0.5);
        }, zoomOut: function () {
            return this.setZoom(this.actualZoom - this.zoomStep);
        },
